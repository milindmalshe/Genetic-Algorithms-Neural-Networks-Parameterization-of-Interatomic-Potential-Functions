...
%Modifying the Milind Malshe's code for Multicomponent system
%Rutuparna Narulkar
function [Vij] = tersoff_multi_component_system(variables,dat,config)%GA
R=1.815;
D=0.335;
%%C-C-C
...
%%O-O-O
% param(2,2,2,1)=variables(1);% type_1, type_2, type_3, A making function
% of R
...
for i=1:numMov
    ...
    iMov = i;%GA
     for j=1:numMov%list(config,1)+1 
		...
		%passing params
        A=variables(1)+variables(12)*rij;
        B=variables(2)+variables(13)*rij;
...
% 			rjk= sqrt( (x(jBond)-x(k3Body)).^2 + (y(jBond)-y(k3Body)).^2 + (z(jBond)-z(k3Body)).^2 );
            ...
            %passing params
			A=variables(1)+variables(12)*rik;
            B=variables(2)+variables(13)*rik;
