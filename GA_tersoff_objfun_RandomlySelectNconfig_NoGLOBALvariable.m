...
%         phen(i,10) = phen(i,10)+ i*1;
        ...
        potential(i,NConfigArray(j)) = calc_Tersoff_GA(phen(i,:),dat(NConfigArray(j),:));
%         tersoff_multi_component_system(phen(i,:),dat(NConfigArray(j),:),NConfigArray(j));
        err(i,NConfigArray(j)) = (potential(i,NConfigArray(j)) - target(NConfigArray(j))).^2;
%         err = (potential(i,NConfigArray(j)) -
%         target(NConfigArray(j))).^2;
...
plot_Tersoff_GA(dat,NConfig,3,[2,2,2]);
